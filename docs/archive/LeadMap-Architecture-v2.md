> **Superseded and frozen.** This copy drifted from `docs/architecture.md`
> (missing the `idempotency_keys` table, its relationship row, its index
> row, and its cache-TTL row, all added during Sprint 2) and is kept here
> only for historical reference. `docs/architecture.md` is the single
